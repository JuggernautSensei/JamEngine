...
void Log::Initialize()
{
    ...
    // 콘솔 싱크 생성
    const spdlog::sink_ptr consoleSink = CreateRef<spdlog::sinks::stdout_color_sink_mt>();
    ...
    const spdlog::sink_ptr fileSink  = CreateRef<spdlog::sinks::basic_file_sink_mt>(filepath, true);
    fileSink->set_pattern("[%Y-%m-%d %H:%M:%S.%e] [%l] %v");

    // 두 싱크를 결합하여 로거 생성
    std::vector<spdlog::sink_ptr>               sinks { consoleSink, fileSink };
    const Ref<spdlog::async_logger> logger = CreateRef<spdlog::async_logger>("CORE", sinks.begin(), sinks.end(), spdlog::thread_pool(), spdlog::async_overflow_policy::block);
