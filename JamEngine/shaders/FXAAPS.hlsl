...
float4 PSmain(PS_INPUT input) : SV_TARGET
{
    ...
    ImageFilterInputTexture1.GetDimensions(0, width, height, dump);

    float2 rcpFrame = float2(1.0f / width, 1.0f / height);

#if (FXAA_PRESET == 0) | (FXAA_PRESET == 1) | (FXAA_PRESET == 2)
    FxaaTex tex = { SamplerLinearAnisotropic, ImageFilterInputTexture1 };
#else  
    FxaaTex tex = { samplerLinearClamp, ImageFilterInputTexture1 };
#endif
